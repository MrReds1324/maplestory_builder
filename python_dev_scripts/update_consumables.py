...
PAGES_CATEGORIES: list[tuple[str, str]] = [
    ...
    ("https://maplestory.fandom.com/wiki/Category:Attack_Speed_Boost_Consumables",
     "ConsumableCategory.attackSpeedBoost"),
    ...
    ("https://maplestory.fandom.com/wiki/Category:Ignored_Enemy_Defense_Boost_Consumables",
     "ConsumableCategory.ignoreDefenseBoost"),
    ("https://maplestory.fandom.com/wiki/Category:Meso_Drop_Rate_Boost_Consumables",
     "ConsumableCategory.mesosObtainedBoost"),
    ("https://maplestory.fandom.com/wiki/Category:Item_Drop_Rate_Boost_Consumables",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog='Update Consumable ENUMs',
        description='Parses the old ENUM file for definitions, and then scrapes for new consumable buffs'
    )
    ...
    if not os.path.isfile(args.input):
        print(f"{args.input} does not point to a file")
        return 1
...
def _scrape_enum_page(main_page_url: str, enum_category: str):
    ...
    for link_no, target_link_element in enumerate(target_link_elements, start=1):
        print(f"Parsing {link_no}/{len(target_link_elements)} for page - {main_page_url}")
...
def _scrape_subpage(target_link_element, local_enum_definitions: dict, enum_category: str) -> None:
    try:
        ...
        for table_element in sub_page_target_search.find_all('tbody'):
            ...
            enum_name_parts = real_name.replace("'", '').replace('&', '').replace('(', '').replace(')', '').replace('.', '').replace('%', '').replace('-', '').split(' ')
            ...
            if not formatted_enum_name or formatted_enum_name == "items" or "Growth" in formatted_enum_name:
                continue
            elif formatted_enum_name[0].isdigit():
        ...
        print(f"Failed to Parse ENUM from link {target_link_element}")
        raise e
...
def handle_duration(stat_effects_string: str) -> int:
    ...
    if not match:
        ...
        if 'minutes' in match.group() or 'minute' in match.group():
            return duration * 60
        elif 'seconds' in match.group() or 'second' in match.group():
            return duration
        elif 'hours' in match.group() or 'hour' in match.group():
...
PERCENTAGE_HANDLING = ("StatType.xallStatsPercentage", "StatType.critRate", "StatType.bossDamage", "StatType.damage", "StatType.ignoreDefense", "StatType.expAdditional")
def handle_stat_effects(stat_effects_string: str, target_enum_definition: dict, subpage_name: str):
    ...
    for stat_type, stat_regex in STAT_REGEX_DICT.items():
        ...
        if stat_type in PERCENTAGE_HANDLING:
