...
class _EquippedHexaStats extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text("Equipped Hexa Stats",
            style: Theme.of(context).textTheme.headlineMedium),
        SetSelectButtonRow<HexaStatsProvider>(
          onHoverFunction:
              context.read<DifferenceCalculatorProvider>().compareHexaStatSets,
          onPressed: (int setPosition) =>
              context.read<HexaStatsProvider>().changeActiveSet(setPosition),
          selectorFunction:
              (BuildContext context, HexaStatsProvider hexaStatsProvider) =>
                  hexaStatsProvider.activeHexaStatsSetNumber,
        ),
...
class _HexaStatSelector extends StatelessWidget {
  ...
  List<DropdownMenuItem> getDropdownItemList(
    ...
    var additionalStatCount =
        hexaStatsProvider.activeHexaStatsSet.additionalStatCount;
    ...
    for (HexaStat hexaStat in hexaStatsProvider.allHexaStats.values) {
      ...
      for (MapEntry<int, int?> equippedHexaStat
          in hexaStatsProvider.activeHexaStatsSet.equippedHexaStat.entries) {
        if (equippedHexaStat.key == hexaStatPosition) {
          continue;
        } else if (hexaStat.hexaStatId == equippedHexaStat.value) {
          continue hexaStatFilterloop;
        } else if (hexaStat.selectedStats[1] ==
            hexaStatsProvider
                .allHexaStats[equippedHexaStat.value]?.selectedStats[1]) {
          continue hexaStatFilterloop;
        } else if ((additionalStatCount[hexaStat.selectedStats[2]]?.$1 == 2 &&
                !(additionalStatCount[hexaStat.selectedStats[2]]
                        ?.$2
                        .contains(hexaStatPosition) ??
                    true)) ||
            (additionalStatCount[hexaStat.selectedStats[3]]?.$1 == 2 &&
                !(additionalStatCount[hexaStat.selectedStats[3]]
                        ?.$2
                        .contains(hexaStatPosition) ??
                    true))) {
          continue hexaStatFilterloop;
  ...
  HexaStat? getSelectedHexaStat(HexaStatsProvider hexaStatsProvider) {
    return hexaStatsProvider.activeHexaStatsSet
        .getSelectedHexaStat(hexaStatPosition);
  }
  ...
  Function _curriedOnHover(HexaStat hexaStat) {
    return (BuildContext context) {
      return context
          ...
          .compareHexaStat(context, hexaStat);
    };
  }
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          Container(
            padding: const EdgeInsets.only(bottom: 5),
            child: Text("Hexa Stat Inventory",
                style: Theme.of(context).textTheme.headlineMedium),
          ),
          Expanded(
            child: Container(
              ...
              child: Consumer<HexaStatsProvider>(
                  builder: (context, hexaStatsprovider, child) {
                var allHexaStatList =
                    hexaStatsprovider.allHexaStats.values.toList();
                      ...
                      child: ListTile(
                        title: Row(children: [
                          Container(
                            ...
                            child: Text(
                                "${allHexaStatList[index].hexaStatName} (${allHexaStatList[index].selectedStats[1]?.formattedName ?? 'None'} lvl ${allHexaStatList[index].totalStatLevel}/20)"),
                          ),
                          const Spacer(),
                          TextButton(
                              onPressed: () => hexaStatsprovider
                                  .deleteHexaStat(allHexaStatList[index]),
                              child: const Text("Delete")),
                          TextButton(
                              onPressed: () => context
                                  .read<HexaStatEditingProvider>()
                                  .addEditingHexaStat(
                                      hexaStat: allHexaStatList[index]),
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: Consumer<HexaStatsProvider>(
                builder: (context, hexaStatsProvider, child) {
              var selectedStats =
                  hexaStatsProvider.calculateStats().entries.toList();
              return ListView.builder(
