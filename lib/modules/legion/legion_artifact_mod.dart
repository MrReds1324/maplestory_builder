...
class LegionArtifactCrystalsModule implements Copyable {
  ...
  int calculateUsedAbilityPoints() {
    ...
    for (MapEntry<int, ArtifactCrystal> crystalEntry
        in artifactCrystals.entries) {
  ...
  Map<StatType, num> calculateModuleStats(int activeArtifactCount) {
    ...
    for (MapEntry<int, ArtifactCrystal> artifactCrystalEntry
        in artifactCrystals.entries) {
      ...
      for (StatType? statType
          in artifactCrystalEntry.value.artifactCrystalStats.values) {
        if (statType == null) {
          continue;
        } else {
          artifactStatLevels[statType] = (artifactStatLevels[statType] ?? 0) +
              artifactCrystalEntry.value.artifactCrystalLevel;
        }
      }
    }

    // Then calculate the actual stat value based on max level of 10
    for (MapEntry<StatType, int> artifactStatLevelsEntry
        in artifactStatLevels.entries) {
      artifactStats[artifactStatLevelsEntry.key] =
          ARTIFACT_STAT_INCREMENTS[artifactStatLevelsEntry.key]! *
              min(artifactStatLevelsEntry.value, MAX_ARTIFACT_STAT_LEVEL);
    }
  ...
  bool addArtifactLevel(
      int artifactCrystalPosition, int availableAbilityPoints) {
    ...
    if (crystalTarget.artifactCrystalLevel == MAX_ARTIFACT_CRYSTAL_LEVEL) {
      return false;
    } else if (ARTIFACT_CRYSTAL_LEVEL_COST[
    ...
    crystalTarget.artifactCrystalLevel += 1;
    crystalTarget.artifactCrystalCost +=
        ARTIFACT_CRYSTAL_LEVEL_COST[crystalTarget.artifactCrystalLevel]!;
  ...
  void updateArtifactStat(
      int artifactCrystalPosition, int statPosition, StatType? statType) {
    artifactCrystals[artifactCrystalPosition]!
        .artifactCrystalStats[statPosition] = statType;
    cacheValue = null;
  }
}
...
class ArtifactCrystal implements Copyable {
  ...
  Widget buildLevelWidget(BuildContext context) {
    ...
    return Row(
        mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: starGroup);
  }

  Widget getArtifactImage(
      BuildContext context, int artifactCrystalPosition, bool enabled) {
    String assetPath;
    if (enabled) {
      int levelToPart;
      ...
      assetPath =
          "assets/images/legion_artifacts/$artifactCrystalPosition.$levelToPart.png";
    } else {
      assetPath =
          "assets/images/legion_artifacts/$artifactCrystalPosition.disabled.png";
    }
