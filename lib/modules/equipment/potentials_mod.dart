  ...
  void _updateStatFromPotential(PotentialLine? potentialLine) {
    if (potentialLine == null) {
      ...
      if (potentialLine.potentialName.potentialType == PotentialType.static) {
        assert(potentialLine.potentialName.statValue is num,
            "Static potential lines must be of type num");
        statValue = potentialLine.potentialName.statValue;
      } else if (potentialLine.potentialName.potentialType ==
          PotentialType.range) {
        assert(potentialLine.potentialName.statValue is List,
      ...
      switch (potentialLine.statType) {
        case StatType.ignoreDefense:
          moduleStats[potentialLine.statType] = calculateIgnoreDefense(
              (moduleStats[potentialLine.statType] ?? 0), statValue);
        case StatType.skill:
        // TODO: add skill stuff here
        default:
          moduleStats[potentialLine.statType] =
              (moduleStats[potentialLine.statType] ?? 0) + statValue;
      }
    }
  }
      ...
      if (potentialLine == null) {
        ...
        if (potentialLine.potentialName.potentialType == PotentialType.static) {
          ...
          assert(potentialLine.potentialName.statValue is List,
              "Range potential lines must be of type num");
          valueToDisplay =
              potentialLine.potentialName.statValue[potentialOffset];
        } else {
          // TODO: add skill stuff here
        }
        ...
        childrenWidgets.add(
          RichText(
            ...
            TextSpan(
                text:
                    "${potentialLine.statType.formattedName} +${potentialLine.statType.isPercentage ? doubleRoundPercentFormater.format(valueToDisplay) : valueToDisplay}",
      ...
      childrenWidgets.add(RichText(
        ...
        TextSpan(
            ...
            style: Theme.of(context)
                .textTheme
                .bodyMedium
                ?.copyWith(color: mainPotential!.color))
      ])));
      ...
      childrenWidgets.add(RichText(
        ...
        TextSpan(
            ...
            style: Theme.of(context)
                .textTheme
                .bodyMedium
                ?.copyWith(color: bonusPotential!.color))
      ])));
...
List<PotentialLine> getPotentialsListForEquip(Equip editingEquip,
    {bool isBonus = false}) {
  ...
  if (isBonus) {
    switch (editingEquip.equipName.equipType) {
      case EquipType.hat:
        filteredList =
            BONUS_HAT_POTENTIALS[editingEquip.potentialModule?.mainPotential] ??
                [];
      ...
      case EquipType.shoulder:
        filteredList =
            BONUS_TOP_OVERALL_BOTTOM_SHOES_CAPE_BELT_SHOULDER_POTENTIALS[
                    editingEquip.potentialModule?.mainPotential] ??
                [];
      case EquipType.gloves:
        filteredList = BONUS_GLOVES_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      case EquipType.face:
      case EquipType.eye:
      ...
      case EquipType.earrings:
        filteredList = BONUS_FACE_EYE_RING_PENDANT_EARRINGS_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      case EquipType.weapon:
        filteredList = BONUS_WEAPON_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      case EquipType.secondary:
      case EquipType.shield:
      case EquipType.katara:
        filteredList = BONUS_SECONDARY_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      case EquipType.emblem:
        filteredList = BONUS_EMBLEM_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      case EquipType.heart:
      case EquipType.badge:
        filteredList = BONUS_HEART_BADGE_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      default:
        return [];
    ...
    switch (editingEquip.equipName.equipType) {
      case EquipType.hat:
        filteredList =
            HAT_POTENTIALS[editingEquip.potentialModule?.mainPotential] ?? [];
      case EquipType.overall:
      case EquipType.top:
        filteredList = TOP_OVERALL_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      case EquipType.bottom:
        filteredList =
            BOTTOM_POTENTIALS[editingEquip.potentialModule?.mainPotential] ??
                [];
      case EquipType.gloves:
        filteredList =
            GLOVES_POTENTIALS[editingEquip.potentialModule?.mainPotential] ??
                [];
      case EquipType.shoes:
        filteredList =
            SHOES_POTENTIALS[editingEquip.potentialModule?.mainPotential] ?? [];
      case EquipType.cape:
      case EquipType.belt:
      case EquipType.shoulder:
        filteredList = CAPE_BELT_SHOULDER_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      case EquipType.face:
      case EquipType.eye:
      ...
      case EquipType.pendant:
      case EquipType.earrings:
        filteredList = FACE_EYE_RING_PENDANT_EARRINGS_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      case EquipType.weapon:
      case EquipType.shield:
      case EquipType.katara:
      case EquipType.secondary:
        filteredList = WEAPON_SECONDARY_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      case EquipType.emblem:
        filteredList =
            EMBLEM_POTENTIALS[editingEquip.potentialModule?.mainPotential] ??
                [];
      case EquipType.heart:
      case EquipType.badge:
        filteredList = HEART_BADGE_POTENTIALS[
                editingEquip.potentialModule?.mainPotential] ??
            [];
      default:
        return [];
  ...
  filteredList += isBonus
      ? BONUS_DEFAULT_POTENTIALS[
              editingEquip.potentialModule?.bonusPotential] ??
          []
      : DEFAULT_POTENTIALS[editingEquip.potentialModule?.mainPotential] ?? [];
