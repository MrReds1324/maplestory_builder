...
class Equip implements Copyable {
  ...
  Equip({
    ...
    if (starForceModule != null) {
      ...
      starForceModule = StarForceModule(
          possibleStars:
              StarForceModule.getStarforceLimit(equipName.itemLevel).toInt());
    } else {
    ...
    if (potentialModule != null) {
      ...
      potentialModule = PotentialModule(
          potentialOffset:
              getPotentialOffsetFromItemLevel(equipName.itemLevel));
    } else {
      potentialModule = null;
    }
    ...
    if (soulModule != null) {
      soulModule = soulModule;
    } else if (equipName.equipType == EquipType.weapon &&
        equipName.itemLevel >= 75) {
      soulModule = SoulModule();
    }
  ...
  Equip copyWith({
    ...
    return Equip(
      ...
      pitchedBossUpgradeModule:
          pitchedBossUpgradeModule ?? this.pitchedBossUpgradeModule?.copyWith(),
  ...
  num getTotalStat(StatType statType) {
    switch (statType) {
      ...
      case StatType.luk:
        return get(statType) +
            get(StatType.allStats) +
            (potentialModule?.get(StatType.allStats) ?? 0) +
            (potentialModule?.get(statType) ?? 0) +
            (starForceModule?.get(statType) ?? 0) +
            (flameModule?.get(statType) ?? 0) +
            (scrollModule?.get(statType) ?? 0) +
            (pitchedBossUpgradeModule?.get(StatType.allStats) ?? 0) +
            (pitchedBossUpgradeModule?.get(statType) ?? 0) +
            (soulModule?.get(StatType.allStats) ?? 0) +
            (soulModule?.get(statType) ?? 0) +
            (tweakModule?.get(statType) ?? 0);
      case StatType.attack:
      case StatType.mattack:
        return get(statType) +
            get(StatType.attackMattack) +
            (potentialModule?.get(statType) ?? 0) +
            (starForceModule?.get(statType) ?? 0) +
            (flameModule?.get(statType) ?? 0) +
            (scrollModule?.get(statType) ?? 0) +
            (pitchedBossUpgradeModule?.get(StatType.attackMattack) ?? 0) +
            (pitchedBossUpgradeModule?.get(statType) ?? 0) +
            (soulModule?.get(statType) ?? 0) +
            (tweakModule?.get(statType) ?? 0);
      case StatType.hp:
      case StatType.mp:
        return get(statType) +
            ...
            (flameModule?.get(statType) ?? 0) +
            (scrollModule?.get(statType) ?? 0) +
            (pitchedBossUpgradeModule?.get(StatType.hpMp) ?? 0) +
            (pitchedBossUpgradeModule?.get(statType) ?? 0) +
            (soulModule?.get(statType) ?? 0) +
            (tweakModule?.get(statType) ?? 0);
      case StatType.ignoreDefense:
      case StatType.ignoreElementalDefense:
        return calculateIgnoreDefenseFromList([
          get(statType),
          (starForceModule?.get(statType) ?? 0),
          (flameModule?.get(statType) ?? 0) +
              (potentialModule?.get(statType) ?? 0),
  ...
  Map<StatType, num> calculateStats() {
    return <StatType, num>{
      ...
      StatType.ignoreElementalDefense:
          getTotalStat(StatType.ignoreElementalDefense),
      ...
      StatType.skillCooldownPercentage:
          getTotalStat(StatType.skillCooldownPercentage),
    };
  }
  ...
  Row createEquipContainer(BuildContext context,
      {bool isEquipEditing = false}) {
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
        ...
        child: Column(
          ...
          children: [
            equipName.starForceCategory == StarForceCategory.none
                ? const SizedBox.shrink()
                : __createTextLine(context, StatType.starForce),
            Center(
                child: Text(
                    "${equipName.formattedName}${(scrollModule?.usedScrolls.length ?? 0) > 0 ? ' +${scrollModule?.usedScrolls.length}' : ''}",
                    style: Theme.of(context).textTheme.headlineSmall)),
            equipName.isUniqueItem
                ? const Center(
                    child: Text("Unique Equipped Item",
                        style: TextStyle(color: EQUIP_UNIQUE_COLOR)))
                : const SizedBox.shrink(),
            ...
            isEquipEditing &&
                    equipName.potentialCategory == PotentialCategory.player
                ? const SizedBox.shrink()
                : potentialModule?.buildPotentialWidget(context, this) ??
                    const SizedBox.shrink(),
          ],
        ),
      ),
  ...
  Widget createSetEffectContainer(BuildContext context,
    ...
    if (isEquipComparing) {
      setEffect = context
          ...
          .diffCalculatorProvider
          .equipsProvider
          .activeEquipSet
          .setEffectModule
          .activeSetEffects[equipSet];
    } else {
      setEffect = context
              .read<EquipsProvider>()
              .activeEquipSet
              .setEffectModule
              .activeSetEffects[equipSet] ??
          (equipSet != null ? SetEffect(equipSet: equipSet!) : null);
    }
    return setEffect?.createSetEffectContainer(context,
            addingEquip: isAdding && isEquipComparing ? this : null,
            removingEquip: isRemoving && isEquipComparing ? this : null) ??
        const SizedBox.shrink();
  }
  ...
  Widget __createTextLine(BuildContext context, StatType statType) {
    ...
    switch (statType) {
      case StatType.starForce:
        return starForceModule?.buildStarForceWidget() ??
            const SizedBox.shrink();
      case StatType.level:
        List<TextSpan> levelText = [];
        if ((flameModule?.get(StatType.level) ?? 0) > 0) {
          levelText.add(TextSpan(
              text:
                  "Required Level: ${max(equipName.itemLevel - (flameModule?.get(StatType.level) ?? 0), 0).toInt()}",
      ...
      case StatType.luk:
        ...
        scrollStat = (scrollModule?.get(statType) ?? 0) +
            (scrollModule?.get(StatType.allStats) ?? 0) +
            (pitchedBossUpgradeModule?.get(StatType.allStats) ?? 0) +
            (pitchedBossUpgradeModule?.get(statType) ?? 0);
    ...
    if (totalStat != baseStat) {
      childrenText.add(TextSpan(
          text:
              "${statType.formattedName}: ${totalStat > 0 ? "+" : ""}${isPercentage ? doubleRoundPercentFormater.format(totalStat) : totalStat} ",
          style: const TextStyle(color: EQUIP_ENHANCED_COLOR)));
      childrenText.add(TextSpan(
        text:
            "(${isPercentage ? doubleRoundPercentFormater.format(baseStat) : baseStat}",
      ));
      if (flameStat != 0) {
        childrenText.add(TextSpan(
            text:
                " +${isPercentage ? doubleRoundPercentFormater.format(flameStat) : flameStat}",
            style: const TextStyle(color: EQUIP_FLAME_COLOR)));
      }
      if (scrollStat != 0) {
        childrenText.add(TextSpan(
            text:
                " ${scrollStat > 0 ? '+' : ''}${isPercentage ? doubleRoundPercentFormater.format(scrollStat) : scrollStat}",
            style: const TextStyle(color: EQUIP_SCROLL_COLOR)));
      }
      if (tweakStat != 0) {
        childrenText.add(TextSpan(
            text:
                " ${tweakStat > 0 ? '+' : ''}${isPercentage ? doubleRoundPercentFormater.format(tweakStat) : tweakStat}",
            style: TextStyle(
                color: tweakStat > 0
                    ? EQUIP_ENHANCED_COLOR
                    : EQUIP_REDUCTION_COLOR)));
      }
      if (starForceStat != 0) {
        childrenText.add(TextSpan(
            text:
                " +${isPercentage ? doubleRoundPercentFormater.format(starForceStat) : starForceStat}",
            style: const TextStyle(color: EQUIP_STAR_COLOR)));
      }
      childrenText.add(const TextSpan(
        text: ")",
      ));
    } else if (baseStat != 0) {
      childrenText.add(TextSpan(
        text:
            "${statType.formattedName}: +${isPercentage ? doubleRoundPercentFormater.format(totalStat) : totalStat}",
      ));
    }
    ...
    if (childrenText.isEmpty) {
      ...
      return RichText(
        text: TextSpan(
            style: DefaultTextStyle.of(context).style, children: childrenText),
      );
    }
  }
...
final List<Equip> equipList = [
  ...
  // Dawn Boss Set Items
  Equip(
      equipName: EquipName.dawnGuardianAngelRing,
  ...
  Equip(
      equipName: EquipName.superiorGolluxRing,
      equipSet: EquipSet.superiorGollux),
  Equip(
      equipName: EquipName.superiorGolluxPendant,
      equipSet: EquipSet.superiorGollux),
  Equip(
      equipName: EquipName.superiorGolluxBelt,
      equipSet: EquipSet.superiorGollux),
  Equip(
      equipName: EquipName.superiorGolluxEarrings,
      equipSet: EquipSet.superiorGollux),
  // Eternal Bowman Items
  Equip(
      equipName: EquipName.eternalArcherHat,
      equipSet: EquipSet.eternalSetBowman),
  Equip(
      equipName: EquipName.eternalArcherHood,
      equipSet: EquipSet.eternalSetBowman),
  Equip(
      equipName: EquipName.eternalArcherPants,
      equipSet: EquipSet.eternalSetBowman),
  Equip(
      equipName: EquipName.eternalArcherShoulder,
      equipSet: EquipSet.eternalSetBowman),
  // Genesis Weapons
  Equip(
      equipName: EquipName.genesisCrossbow,
      equipSet: EquipSet.eternalSetBowman,
      starForceModule: StarForceModule(possibleStars: 25, currentStars: 22)),
  // Arcane Bowman Items
  Equip(
      equipName: EquipName.arcaneUmbraArcherHat,
      equipSet: EquipSet.arcaneSetBowman),
  Equip(
      equipName: EquipName.arcaneUmbraArcherSuit,
      equipSet: EquipSet.arcaneSetBowman),
  Equip(
      equipName: EquipName.arcaneUmbraArcherShoes,
      equipSet: EquipSet.arcaneSetBowman),
  Equip(
      equipName: EquipName.arcaneUmbraArcherShoulder,
      equipSet: EquipSet.arcaneSetBowman),
  Equip(
      equipName: EquipName.arcaneUmbraArcherGloves,
      equipSet: EquipSet.arcaneSetBowman),
  Equip(
      equipName: EquipName.arcaneUmbraArcherCape,
      equipSet: EquipSet.arcaneSetBowman),
  // Arcane Weapons
  Equip(
      equipName: EquipName.arcaneUmbraCrossbow,
      equipSet: EquipSet.arcaneSetBowman),
  // Pitched Boss Set Items
  Equip(
      equipName: EquipName.blackHeart,
      equipSet: EquipSet.pitchedBoss,
      potentialModule: PotentialModule(
          potentialOffset: 0,
          mainPotential: PotentialTier.epic,
          mainPotentials: const {
            1: PotentialLine(
  ...
  Equip(
      ...
      equipSet: EquipSet.pitchedBoss,
      pitchedBossUpgradeModule: PitchedBossUpgradeModule(
          pitchedBossUpgrade: PitchedBossUpgrade.gravityModule)),
  ...
  Equip(
      equipName: EquipName.cursedRedSpellbook, equipSet: EquipSet.pitchedBoss),
  Equip(
      equipName: EquipName.cursedGreenSpellbook,
      equipSet: EquipSet.pitchedBoss),
  Equip(
      equipName: EquipName.cursedBlueSpellbook, equipSet: EquipSet.pitchedBoss),
  Equip(
      equipName: EquipName.cursedYellowSpellbook,
      equipSet: EquipSet.pitchedBoss),
  Equip(
      equipName: EquipName.commandingForceEarring,
      equipSet: EquipSet.pitchedBoss),
  ...
  Equip(
      ...
      equipSet: EquipSet.pitchedBoss,
      pitchedBossUpgradeModule: PitchedBossUpgradeModule(
          pitchedBossUpgrade: PitchedBossUpgrade.nightmareFragment)),
  ...
  Equip(
      equipName: EquipName.mitrasRageMagician, equipSet: EquipSet.pitchedBoss),
