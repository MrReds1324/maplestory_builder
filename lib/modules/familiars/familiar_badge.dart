...
class FamiliarBadge {
  ...
  static FamiliarBadge loadFromJson(
      int badgeId, Map<String, dynamic> jsonData) {
    String badgeName = jsonData["badgeName"];
    Map<StatType, num> badgeStats = loadJsonStatMap(jsonData["badgeStats"]);
    List<Map<StatType, num>> randomStats =
        loadJsonRandomStats(jsonData["randomStats"]);

    return FamiliarBadge(
        badgeName: badgeName,
        badgeId: badgeId,
  ...
  Widget createBadgeContainer(BuildContext context) {
    Widget assetImage = Image(
        image: AssetImage('assets/images/familiar_badges/$badgeId.png'),
        height: 105,
    ...
    for (MapEntry<StatType, num> statEntry in badgeStats.entries) {
      statWidgets.add(statEntry.key.buildRowDisplayWithValue(
          context, statEntry.value, 150,
