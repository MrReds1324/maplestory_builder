...
class LegionStatsProvider with ChangeNotifier implements Copyable {
  ...
  LegionStatsProvider({
    ...
    this.legionSets = legionSets ??
        {
          1: LegionModule(
              getLegionBoardRankCallback: getLegionBoardRankCallback,
              getLegionCharacterCallback: getLegionCharacterCallback),
          2: LegionModule(
              getLegionBoardRankCallback: getLegionBoardRankCallback,
              getLegionCharacterCallback: getLegionCharacterCallback),
          3: LegionModule(
  ...
  LegionStatsProvider copyWith(
    ...
    return LegionStatsProvider(
      ...
      allLegionCharacters: allLegionCharacters ??
          Map<int, LegionCharacter>.of(this.allLegionCharacters),
  ...
  LegionStatsProvider update(CharacterProvider characterProvider) {
    allLegionCharacters[0]!.legionCharacterLevel =
        characterProvider.characterLevel;
    allLegionCharacters[0]!.legionBlock =
        characterProvider.characterClass.legionBlock;
    calculateLegionBoardRank();
    calculateStats();
    notifyListeners();
    return this;
  ...
  void addLegionStatLevels(int levelsToAdd, StatType statType,
      {bool isOuterBoard = false}) {
    if (activeLegionSet.addLegionStatLevels(levelsToAdd, statType,
        isOuterBoard: isOuterBoard)) {
      notifyListeners();
    }
  }

  void subtractLegionStatLevels(int levelsToSubtract, StatType statType,
      {bool isOuterBoard = false}) {
    if (activeLegionSet.subtractLegionStatLevels(levelsToSubtract, statType,
        isOuterBoard: isOuterBoard)) {
      notifyListeners();
    }
  }
  ...
  void calculateLegionBoardRank() {
    var sortedCharacters = allLegionCharacters.values.toList()
      ..sort(LegionCharacter.comparator);
    sortedCharacters = sortedCharacters
        .where((element) =>
            !LegionBlock.SPECIAL_BLOCKS.contains(element.legionBlock))
        .toList();
    sortedCharacters =
        sortedCharacters.sublist(0, min(42, sortedCharacters.length));
    totalCharacterLevels = sortedCharacters.fold(
        0,
        (int previous, LegionCharacter current) =>
            previous + current.legionCharacterLevel);
  ...
  void saveEditingLegionCharacter(LegionCharacter? editingLegionCharacter) {
    ...
    if (editingLegionCharacter.legionCharacterHash == -1) {
      ...
      allLegionCharacters[editingLegionCharacter.legionCharacterHash] =
  ...
  List<LegionCharacter> getUnplacedLegionCharacters() {
    ...
    for (LegionCharacter legionCharacter in allLegionCharacters.values) {
      if (!placedLegionCharacters
          .contains(legionCharacter.legionCharacterHash)) {
